...
class MantisIssueAddRelationshipV1
  ...
  def initialize(input)
	...
	#Build a hash that maps the names of the relationship types to the appropriate identifier
	@relationships = {
		:duplicate_of => 0,
		:related_to => 1,
		:parent_of => 2,
  ...
  def execute
    begin
      ...
      # raised.
      response = @client.request 'wsdl:mc_issue_relationship_add' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issue_id' => @parameters['issue_id'], 'relationship' => {
				'type' => {'id' => @relationships[@parameters['relationship_type_id'].downcase.gsub(/ /,"_").to_sym]},
				'target_id' => @parameters['relationship_target_id']
			}
		}
      end
    ...
    res = response.to_hash[:mc_issue_relationship_add_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Relationship_ID">#{res}</result>
    </results>
    RESULTS
  end
