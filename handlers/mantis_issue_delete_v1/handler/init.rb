...
class MantisIssueDeleteV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_delete' do |soap_message|
    ...
    res = response.to_hash[:mc_issue_delete_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Delete_Success">#{res}</result>
    </results>
    RESULTS
  end
