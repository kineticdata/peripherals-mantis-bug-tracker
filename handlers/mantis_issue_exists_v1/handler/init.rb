...
class MantisIssueExistsV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_exists' do |soap_message|
    ...
    res = response.to_hash[:mc_issue_exists_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Exists">#{res}</result>
    </results>
    RESULTS
  end
