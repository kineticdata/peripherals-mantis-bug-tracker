...
class MantisFetchIssueDataV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_get' do |soap_message|
    ...
    res = response.to_hash[:mc_issue_get_response][:return]
	#Generate JSON formatted string from the retrieved data.
	res = JSON.generate(res)

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Data">#{res}</result>
    </results>
    RESULTS
  end
