...
class MantisProjectDeleteCategoryV1
  ...
  def execute
    begin
      # Build and send a SOAP request for the 'mc_project_add' method (namespaced with
      ...
      response = @client.request 'wsdl:mc_project_delete_category' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'project_id' => @parameters['project_id'].to_i,
			'p_category_name' => @parameters['category_name']
		}
      end
    ...
    res = response.to_hash[:mc_project_delete_category_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Category_Delete_Success">#{res}</result>
    </results>
    RESULTS
  end
