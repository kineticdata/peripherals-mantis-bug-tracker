...
class MantisIssueCheckInV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_checkin' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issue_id' => @parameters['issue_id'].to_i, 'comment' => @parameters['comment'], 'fixed' => @parameters['fixed']
		}
      end
    ...
    res = response.to_hash[:mc_issue_checkin_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Check_In_Success">#{res}</result>
    </results>
    RESULTS
  end
