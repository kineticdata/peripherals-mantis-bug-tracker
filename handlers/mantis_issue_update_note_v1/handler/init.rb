...
class MantisIssueUpdateNoteV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_note_update' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'note' => {
				'id' => @parameters['note_id'].to_i,
				'text' => @parameters['note_text'],
				'view_state' => @parameters['view_state'].downcase
			}
		}
      end
    ...
    res = response.to_hash[:mc_issue_note_update_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Note_Update_Success">#{res}</result>
    </results>
    RESULTS
  end
