...
class MantisProjectAddCategoryV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_project_add_category' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'project_id' => @parameters['project_id'], 'p_category_name' => @parameters['category_name']
		}
      end
    ...
    res = response.to_hash[:mc_project_add_category_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Project_Category_ID">#{res}</result>
    </results>
    RESULTS
  end
