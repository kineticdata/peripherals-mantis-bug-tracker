...
class MantisProjectCreateV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_project_add' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'project' => {
				'name' => @parameters['name'],
				'status' => {'name' => @parameters['status'].downcase},
				'enabled' => @parameters['enabled'].downcase,
				'view_state' => {'name' => @parameters['view_state'].downcase},
				'access_min' => {'name' => @parameters['access_min'].downcase},
				'description' => @parameters['description'],
				'inherit_global' => @parameters['inherit_global'].downcase,
			}
		}
      end
    ...
    res = response.to_hash[:mc_project_add_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Project_ID">#{res}</result>
    </results>
    RESULTS
  end
