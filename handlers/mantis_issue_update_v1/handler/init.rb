...
class MantisIssueUpdateV1
  ...
  #
  # ==== Returns
  # A JSON formatted String representing the return variable results.
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_update' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issueId' => @parameters['issue_id'], 'issue' => {
				'project' => {'name' => @parameters['project']},
				'category' => @parameters['category'],
				'summary' => @parameters['summary'],
				'description' => @parameters['description'],
				'priority' => {'name' => @parameters['priority']},
				'severity' => {'name' => @parameters['severity']},
				'status' => {'name' => @parameters['status']},
				'reporter' => {'name' => @parameters['reporter']},
				'version' => @parameters['version'],
				'build' => @parameters['build'],
				'platform' => @parameters['platform'],
				'os' => @parameters['os'],
				'os_build' => @parameters['os_build'],
				'reproducibility' => {'name' => @parameters['reproducibility']},
				'sponsorship_total' => @parameters['sponsorship_total'],
				'handler' => {'name' => @parameters['handler']},
				'projection' => {'name' => @parameters['projection']},
				'eta' => {'name' => @parameters['eta']},
				'resolution' => {'name' => @parameters['resolution']},
				'fixed_in_version' => @parameters['fixed_in_version'],
				'steps_to_reproduce' => @parameters['steps_to_reproduce'],
				'add_info' => @parameters['add_info'],
    ...
    res = response.to_hash[:mc_issue_update_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Update_Successful">#{res}</result>
    </results>
    RESULTS
  end
