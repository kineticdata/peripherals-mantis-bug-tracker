...
class MantisIssueGetAttachmentV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_attachment_get' do |soap_message|
    ...
    res = response.to_hash[:mc_issue_attachment_get_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Attachment_Content">#{res}</result>
    </results>
    RESULTS
  end
