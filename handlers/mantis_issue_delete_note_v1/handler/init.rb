...
class MantisIssueDeleteNoteV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_note_delete' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issue_note_id' => @parameters['note_id'].to_i
		}
      end
    ...
    res = response.to_hash[:mc_issue_note_delete_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Note_Delete_Success">#{res}</result>
    </results>
    RESULTS
  end
