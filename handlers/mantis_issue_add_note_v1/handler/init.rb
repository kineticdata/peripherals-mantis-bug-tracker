...
class MantisIssueAddNoteV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_note_add' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issue_id' => @parameters['issue_id'].to_i, 'note' => {
    ...
    res = response.to_hash[:mc_issue_note_add_response][:return]

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_Note_ID">#{res}</result>
    </results>
    RESULTS
  end
