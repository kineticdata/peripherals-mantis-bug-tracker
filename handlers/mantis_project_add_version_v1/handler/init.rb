...
class MantisProjectAddVersionV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_project_version_add' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'version' => {
				'id' => '',
				'name' => @parameters['version_name'],
				'project_id' => @parameters['project_id'],
				'date_order' => '',
				'description' => @parameters['version_description'],
				'released' => to_boolean(@parameters['released']),
				'obsolete' => to_boolean(@parameters['obsolete'])
			}
		}
      end
    ...
    res = response.to_hash[:mc_project_version_add_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Project_Version_ID">#{res}</result>
    </results>
    RESULTS
  end
