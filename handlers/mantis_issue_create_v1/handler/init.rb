...
class MantisIssueCreateV1
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_add' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issue' => {
    ...
    res = response.to_hash[:mc_issue_add_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Issue_ID">#{res}</result>
    </results>
    RESULTS
  end
