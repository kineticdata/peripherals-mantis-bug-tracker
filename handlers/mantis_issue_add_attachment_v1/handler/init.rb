...
require 'open-uri'
class MantisIssueAddAttachmentV1
  ...
  def initialize(input)
	...
	split_char = @parameters['file_path'].split('\\').length > 1 ? '\\' : '/'
	@file_name = @parameters['file_path'].split(split_char.to_s).last.to_s
	@file_content = Base64.strict_encode64(open(@parameters['file_path']).read)
  ...
  def execute
    begin
      ...
      response = @client.request 'wsdl:mc_issue_attachment_add' do |soap_message|
        ...
        soap_message.body = {
			'username' => @info_values['username'], 'password' => @info_values['password'], 'issue_id' => @parameters['issue_id'].to_i,
			'name' => @file_name, 'file_type' => '', 'content' => @file_content
		}
      end
    ...
    res = response.to_hash[:mc_issue_attachment_add_response][:return].to_i

    # Build and return the results
    <<-RESULTS
    <results>
      <result name="Mantis_Attachment_ID">#{res}</result>
    </results>
    RESULTS
  end
